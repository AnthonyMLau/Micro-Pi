...
int run(int n, char* str) {
	...
	for (int i = 0; i < n; i++) h_a[i] = make_cuDoubleComplex((double)str[n - i - 1] - '0', 0.0);
	for (int i = 0; i < n; i++) h_b[i] = make_cuDoubleComplex((double)str[n - i - 1] - '0', 0.0);
	...
	printf("\r\n");
	while (!h_ans[t - 1]) t--;
	//for (int i = t - 1; i >= 0; i--) printf("%d", h_ans[i]);
	//delete
